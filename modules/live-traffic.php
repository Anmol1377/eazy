...
if (get_option('wpg_live_traffic') == 1) {
	...
	$cache_file = plugin_dir_path(__FILE__) . "/cache/live-traffic/". str_replace(":", "-", $ip) .".json";
	...
	if (ezy_sc_getcache($cache_file) == 'WPGUD_NoCache') {
		...
		@$country_code = ezy_sc_getcache($cache_file);
    ...
    $uvcheck = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table

                    WHERE ip = %s AND useragent = %s AND date = %s LIMIT 1", $ip, $useragent, $date));
    ...
    $vischeck = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table

                    WHERE ip = %s AND useragent = %s AND request_uri = %s AND date = %s AND time = %s LIMIT 1", $ip, $useragent, $request_uri, $date, $time));
