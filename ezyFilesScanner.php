...
define('ezy_SCANNER_MAX_FILE_SIZE', 256*1024);

class CezyFilesScanner
{
    protected $_logger;
    ...
    public function __construct($heuristic=false)
    {
        ...
        $this->_patterns_db         = new CezyPatternsDatabase();
        $this->_report              = new CezyReport();
        $this->_config              = new CezyConfig(); 
        $this->_stats               = new CezyStats();
        $this->_ignore_list         = new CezyIgnoreList();
        $this->_files_white_list    = new CezyFilesWhiteList();
        $this->_mime_filer          = new CezyMimetype(); 
    ...
    public function ScanFile($path)
    {
        ...
        if( strpos($path,"runtime.log") or strpos($path,"ezy_sc_wp_report.txt")){
            /*
        ...
        if( $size > ezy_SCANNER_MAX_FILE_SIZE){
            $this->_stats->IncClean();
            $this->_logger->Info(sprintf("[%s] is clean", $path));
            return FALSE;
        }
        ...
        /*
         * matches is map between pattern (CezyPattern) and found match
         */
    ...
    protected function _ScanFile($path)
    {
        ...
        if( strpos($path,"ezy_sc_wp_report") ){
            /*
             * skip investigation generated report
             */
            $this->_stats->IncClean();
    ...
    private function _ReloadCoreMap(){
        ...
        $checksums_data = CezyUtils::GetUrlContent($apiurl);
